...
def draw_loss_graph(event1,event2,event3):
    """
    绘制吞吐量曲线
    :return: 
    """
    ...
    np.savetxt(os.path.join(tensorlog_save_path,'delay-itera-HARQ-IR4.csv'), df2)
    #
    df1 = smooth(df3_train_loss , weight= 0.6)
    df2 = df1.loc[:, 'delay'].to_numpy()
    df2 = df2.reshape(1, -1).T
    np.savetxt(os.path.join(tensorlog_save_path,'delay-itera-HARQ-CC4.csv'), df2)


    df1 = smooth(df1_train_loss,weight=0.6)
    df2 = df1.loc[:, 'delay'].to_numpy()
    df2 = df2.reshape(1, -1).T
    np.savetxt(os.path.join(tensorlog_save_path,'delay-itera-HARQ4.csv'), df2)
...
if __name__ == '__main__':
    event1 = os.path.join(tensorlog_save_path,'events.out.tfevents.1738809060.MIRAGE.15652.0')
    event2 = os.path.join(tensorlog_save_path,'events.out.tfevents.1738809513.MIRAGE.22540.0')
    event3 = os.path.join(tensorlog_save_path,'events.out.tfevents.1738825348.MIRAGE.20720.0')

    draw_loss_graph(event1,event2,event3)
    #draw_graph()
