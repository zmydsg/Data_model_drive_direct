...
def getSolution(NumK, modelname, base=''):
    ...
    f = open(f"testlog\\{modelname}_{NumK}_power{base}.pk", 'rb')
    data = pickle.load(f)
...
def draw_power_graph(NumKs, PDBs):
    ...
    # print("=" * 10)
    if not os.path.exists(os.path.join(project_path,f'csv/power_outage-NumK={NumKs}.csv')) or not os.path.exists(os.path.join(project_path,f'csv/power_through-NumK={NumKs}.csv')):
        ...
        df1.to_csv(os.path.join(project_path,f'csv/power_outage-NumK={NumKs}.csv'), na_rep='NaN', header=False, index=False)

        df2 = pd.DataFrame(throughListToMatlab)
        df2.to_csv(os.path.join(project_path,f"csv/power_through-NumK={NumKs}.csv"), na_rep='NaN', header=False, index=False)
    ...
    plt.savefig(os.path.join(graph_path, f'eta(bpsHz)-NumK={NumKs}.jpg'))
    ...
    plt.legend([u"Type-I HARQ", u"HARQ-CC", u"HARQ-IR"])
    plt.savefig(os.path.join(graph_path, f'bar-p_out-asy,K-NumK={NumKs}.jpg'))
    plt.show()
...
def draw_auto_pdb_power_graph(NumKs, PDBs):
    ...
    if not os.path.exists(os.path.join(project_path,f'csv/power_outage-NumK={NumKs}_avg.csv')) or not os.path.exists(os.path.join(project_path,f'csv/power_through-NumK={NumKs}_avg.csv')):
        outageListToMatlab = [GNN_out_HARQ, GNN_out_HARQ_CC, GNN_out_HARQ_IR]
        throughListToMatlab = [GNN_delay_HARQ, GNN_delay_HARQ_CC, GNN_delay_HARQ_IR]
        df1 = pd.DataFrame(outageListToMatlab)
        df1.to_csv(os.path.join(project_path,f'csv/power_outage-NumK={NumKs}_avg.csv'), na_rep='NaN', header=False, index=False)

        df2 = pd.DataFrame(throughListToMatlab)
        df2.to_csv(os.path.join(project_path,f"csv/power_through-NumK={NumKs}_avg.csv"), na_rep='NaN', header=False, index=False)
