...
def generate2(factors, K,x):
    factor1,factor2=factors
    for k in range(K):
        for m in range(K):
            x[k, m] = factor1**k * factor2**m
            if k == m:
                x[k, m] += np.sqrt((1.-factor1**(2*k)) * (1.-factor2**(2*m)))
    return x
def generate1(factors,K,x):
    for i, factor1 in enumerate(factors):
        for u, factor2 in enumerate(factors):
            if i == u:
                x[i, u] = 1
                continue
            hk = c = 0
            for k in range(1,K+1):
                for m in range(1,1+K):
                    for j in range(-K,K+1):
                        if k == m+j:
                            hk += np.sqrt((1. - factor1**(2*k))*(1. - factor2**(2*m)));c+=1
                        if j == 0:
                            hk += factor1**k * factor2**m;c+=1
                        if k == j:
                            hk += np.sqrt(1. - factor1**(2*k)) * factor2**m;c+=1
                        if m + j == 0:
                            hk += factor1**k * np.sqrt(1. - factor2**(2*m));c+=1
            x[i, u] = hk / c#(K*K*K+K)
    return x
...
if __name__=='__main__':
    ...
    generateDataSet(train_data_name, 2000, args.train_seed, replace=True)#500
