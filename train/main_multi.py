...
def train(eval=False):
    ...
    for model_name in model_name_list:
        ...
        # 4. 修改模型初始化
        model_pdm = HybridModel(
            model_name=model_name,
            gcn_model=gcnmodel,
            ddpg_model=ddpg_agent,
            environment=environment,
            Numk=NumK,
            PoutFactor=0.1,  # 添加中断概率因子
            constraints={'pout': bounds, 'power': PDBS},  # 添加约束条件
            train_size=batch_size,  # 添加训练批次大小
            device=device,
            learning_rates=update_Step  # 添加学习率
        ).to(device)
