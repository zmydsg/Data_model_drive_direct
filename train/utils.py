...
def load_data(dpath, **kwargs):
    ...
    with h5py.File(dpath,'r') as handle:
        ...
        # 获取第一个样本的维度作为标准
        edge_index, h = dense_to_sparse(torch.from_numpy(
            hs[0,0:-1].reshape(reshapesize,reshapesize).astype(float)))
        standard_dim = len(h)
        
        for index in range(datanum1):
            factor = hs[index,-1]
            edge_index, h = dense_to_sparse(torch.from_numpy(
                hs[index,0:-1].reshape(reshapesize,reshapesize).astype(float)))
            
            # 确保h的维度一致
            if len(h) != standard_dim:
                print(f"跳过样本 {index}: 维度不匹配 (expected {standard_dim}, got {len(h)})")
                continue
    ...
    try:
        X = np.concatenate(X, axis=0)  # 沿batch维度连接
    except ValueError as e:
        print("数据维度不匹配:")
        for i, x in enumerate(X):
            print(f"样本 {i} 形状: {x.shape}")
        raise e
...
def getdata(data_path, pdb, NumK, eval=False, **kwargs):
    """
    读取产生的数据，并将初始功率拼接上去
    """
    ...
    listtoprocess = ['tr','val'] if eval else ['tr']
...
def edge_index_batch(edgeindex, numhx, NumK, dev):
    ...
    listshift = torch.arange(0, numhx*NumK, NumK)[:, None].long().repeat(2, 1, edgeindex.shape[1]).view(2, -1).to(dev)
    #无向图
    ...
    # print(listshift.max(), edgeindex_batch.max())
...
one_power = lambda i: -1 if i&1 else 1

def funct_cal_HARQ_IR(result, pt, factor, NumK, rate_2, i=1):
    """
    HARQ_IR 中断概率计算
    """
    ...
    for i in range(i,NumK+1):  # 1， 2， 3，..., K
        # print(f"i:{i}")

        # A : GK(x)
        a0 =0
        for j in range(i):     # 0, 1, 2, ..., K-1
            a1 = math.factorial(i-j-1)
            a0 += one_power(j)*(a2**(i-j-1))/a1
        a3 = one_power(i)+rate_2*a0
...
def funct_cal_HARQ_CC(result, pt, factor, NumK, rate_2, i=1):
    """
    HARQ_CC 中断概率计算
    """
    f2 = factor*factor
    for i in range(i,NumK+1):
        a0 = (rate_2-1) ** i
        a1 = math.factorial(i)
        # L(factor ,K)
        a4 = 1
...
def funct_cal_HARQ(result, pt, factor, NumK, rate_2, i=1):
    """
    1型 HARQ 中断概率计算
    """
    f2 = factor*factor
...
def get_utility_func_(m):
    return {
        'HARQ-IR': funct_cal_HARQ_IR_,
        'HARQ-CC': funct_cal_HARQ_CC_,
        'HARQ': funct_cal_HARQ_,
    }[m]

def funct_cal_HARQ_IR_(result, PoutK, pt, sr, factor, NumK, rate_2, roi):
    a2 = math.log(rate_2, math.e)
    f2 =factor*factor
...
def funct_cal_HARQ_CC_(result, PoutK, pt, sr, factor, NumK, rate_2, roi):
    f2 = factor*factor
    for i in range(i,NumK+1):
        a0 = (rate_2-1) ** i
        a1 = math.factorial(i)
...
def funct_cal_HARQ_(result, PoutK, pt, sr, factor, NumK, rate_2, roi):
    f2 = factor*factor
    for i in range(i,NumK+1):
        a0 = (rate_2 - 1) ** i
...
def SINR_D(pas, N, K):
    res = torch.ones((pas.shape[0], K), dtype=torch.float32, requires_grad=False)
    D= 1. + pas.sum(dim=1) - pas[:, N]
    for k in range(K):
        res[:, k] = D*res[:, k-1]
    return res
